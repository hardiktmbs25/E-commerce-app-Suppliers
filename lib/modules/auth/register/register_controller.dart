...
class RegisterController extends GetxController {
  ...
  final otherServiceCtrl = TextEditingController();
  ...
  String? validatePassword(String? v) {
    ...
    if (v.length < 6) return 'Minimum 6 digits required';
    if (!RegExp(r'^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{6,}$').hasMatch(v)) {
  ...
  Future<void> register() async {
    ...
    if (selectedService.value == 'custom' && otherServiceCtrl.text.trim().isEmpty) {
      Get.snackbar('Error', 'Please enter your service name',
          backgroundColor: Colors.red, colorText: Colors.white,
          snackPosition: SnackPosition.BOTTOM);
      return;
    }
    ...
    await authResult.fold(
        ...
        String finalService = selectedService.value == 'custom' 
            ? otherServiceCtrl.text.trim() 
